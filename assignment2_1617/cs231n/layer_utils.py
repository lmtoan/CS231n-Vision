...
def affine_bn_relu_forward(x, w, b, gamma, beta):
    """
    Convenience layer that perorms an affine transform followed by Batch-Norm and ReLU

    Inputs:
    - x: Input to the affine layer
    - w, b: Weights for the affine layer
    - gamma, beta: Shift and Scale for the batch-norm layer

    Returns a tuple of:
    - out: Output from the ReLU
    - cache: Object to give to the backward pass
    """
    affine_out, af_cache = affine_forward(x, w, b)
    bn_param = {'mode': 'train'}
...
def affine_relu_dropout_forward(x, w, b, dropout_param):
    ...
    cache = (af_cache, relu_cache, dropout_cache)
    return out, cache
