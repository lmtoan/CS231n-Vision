...
for lr in np.arange(0.00000001, 0.0000001, 0.00000005):
	for reg in np.arange(1e4, 1e5, 10000):
		print 'LR: %f and REG: %d' % (lr, reg)
		svm_iter = LinearSVM()
		svm_iter.train(X_train, y_train, learning_rate=lr, reg=reg, num_iters=2000, verbose=True)
		y_val_pred_iter = svm_iter.predict(X_val)
		y_train_pred_iter = svm_iter.predict(X_train)
